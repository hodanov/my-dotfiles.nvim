...
- [x] `SKILL.md` の `argument-hint` が `all / gha / dependabot / automerge` に更新されている
- [x] ワークフロー全体像に Phase 4 が追加され、検証が Phase 5 に移動している
- [x] Phase 4-0 の前提チェック手順が記述されている
- [x] Phase 4-1 に Dependabot 向け `auto-merge-deps.yml` テンプレートが含まれている
- [x] Phase 4-2 に Renovate 向け `automerge` 設定例が含まれている
- [x] Phase 5 の検証リストに自動マージの確認項目が追加されている
- [x] `$ARGUMENTS: automerge` でスコープを絞った実行が機能するよう記述されている
